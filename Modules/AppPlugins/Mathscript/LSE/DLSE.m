function [Vestimate, tve, Iji_index]=DLSE(lsecon,measure_matrix,V,network_data,pmu_num)
...
I_array=[];
for i=1:pmu_num
   for j=4:3:size(measure_matrix,2)
       if(measure_matrix(i,j)~=0)
           I_array(h)=measure_matrix(i,j);
           h=h+1;
       end
   end
end
...
error=zeros(1,lsecon.IEEESystem);
for k=1:Num2
    n=Bus(k);
    Vestimate(n)=x(k)+1i*x(k+Num2);
    tve(n)=100*abs(V(n)-Vestimate(n))/abs(V(n));  %Calculate the error between estimated value and the true value
end
