...
clear all
%% Default parameters
timevector=[-10:1/60:10]'; % time vector - I set a long time period to see long term drift behavior
stat=logical([zeros(length(timevector),16)]); % stat message formatted as per IEEE Std. C37.118.2-2011
p_up=.0000005;                  % p_up is the probability of transferring from locked to the unlocked state (1-reliability of the gps system) [http://www.gps.gov/technical/ps/2008-SPS-performance-standard.pdf]
q_down= 0.95;               % q_down is the probability of transferring from unlocked to locked state (redundancy of satellite coverage* recovery probability of the GPS receiver)
