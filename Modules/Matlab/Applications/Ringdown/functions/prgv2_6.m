...
function [identmodel, xcon, plhs_3, plhs_4] =...
    prgv2_6(sigdat, tstep, shftnnft, inpulses, known_modes, xcon)
    ...
    [dampf1, frqrps1, ampopt2, phase2, dmodes, mretr_key,...
    ...
    if mretr_key == 0,
        ...
        %if(isnan(sum(dampf2))||isnan(sum(frqrps2))||isnan(sum(ampopt2))||isnan(sum(phase2))...
%             ||...
%             ~(isequal((size(dampf2)==size(frqrps2))&(size(dampf2)==size(ampopt2))&(size(dampf2)==size(phase2)),[1 1])))
%             identmodel = zeros(39,4);
        %else
            identmodel = [dampf2, frqrps2, ampopt2, phase2];
            
        %end
        %identmodel = reshape(identmodel_temp, length(dampf2), 4);
        xcon = [qcon; scalmod; lpocon; pircon; dmodes; lpmcon; lpacon; fbcon; 0; trimre; ftrimh; ftriml];        
...
%         
%         m = 0;
%         wrn_messages = 0;%wrnflg.err_msg(wrnflg_vect ~= 0);
%         
%         if m == 0,
%             plhs_3 = [];
%         elseif m == 1,
...
%             elseif mretr2 == 101,
%                 plhs_4 = 'Fatal error. Norm of full-order model time domain signal is zero.';
%             else,
%                 plhs_4 = ftlerr.err_msg(ftlerr.num_key == 8);
%             end
%         else,
%             if mretr_key == 100,
%                 plhs_4 = 'Fatal error. Not enough data points.';
%             elseif (mretr_key == 102) | (mretr_key == 103),
%                 plhs_4 = 'Fatal error. No modes identfied.';
%             elseif mretr_key == 104,
%                 plhs_4 = 'Fatal error. Sample period too small.';
%             elseif mretr_key == 105,
%                 plhs_4 = 'Fatal error. Linear prediction order too small.';
%             elseif mretr_key == 110,
%                 plhs_4 = 'Fatal error. Data sequence is a null signal.';
%             else,
%                 plhs_4 = 'Fatal error. Norm of full-order model time domain signal is zero.';
%             end
%         end
