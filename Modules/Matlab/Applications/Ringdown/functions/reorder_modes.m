...
function [amp, damp, frq, phase, aic, releng, mretr] = reorder_modes(time, nnfit,...
    ...
    if ordcon == 1,
        ...
        for ind_i = 1:qconsub            
            relmax = 0;
            releng_aux=releng(ind_i:qconsub);
            if max(releng_aux) > relmax,
                relmax = max(releng_aux);
                find(releng == max(releng_aux));
                isv = find(releng_aux == max(releng_aux)) + ind_i - 1;
            end
            % ==========================
            amptmp = amp(isv);
            damptmp = damp(isv);
